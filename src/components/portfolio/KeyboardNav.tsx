    ...
    <div className="flex items-center justify-between py-2 px-3 rounded-lg bg-white/5 hover:bg-white/10 transition-colors">
      ...
      <div className="flex gap-1">
        {keys.split('+').map((key, i) => (
          <span key={i}>
            <kbd className="px-2 py-1 text-xs font-mono bg-black/50 border border-white/20 rounded text-emerald-400">
              {key}
            </kbd>
          </span>
        ))}
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.key.toLowerCase()) {
        ...
        case 't':
          ...
          showToast(terminalOpen ? 'Terminal Closed' : 'Terminal Opened', <Terminal className="w-4 h-4" />);
          break;
        case 'm':
          setMatrixEnabled(prev => !prev);
          showToast(matrixEnabled ? 'Matrix Rain Disabled' : 'Matrix Rain Enabled');
          break;
        case 's':
          setSoundEnabled(prev => !prev);
          showToast(soundEnabled ? 'Sound Disabled' : 'Sound Enabled', <Volume2 className="w-4 h-4" />);
          break;
        case 'h':
          window.scrollTo({ top: 0, behavior: 'smooth' });
          setCurrentSection(0);
          showToast('Going Home', <Home className="w-4 h-4" />);
          break;
        case '?':
  ...
  return (
    ...
    >
      <motion.button
        ...
        className="fixed bottom-6 right-6 z-50 w-12 h-12 rounded-xl bg-black/80 border border-white/10 flex items-center justify-center group hover:border-cyan-500/60 transition-all shadow-lg backdrop-blur-sm"
        title="Keyboard Shortcuts"
      >
        <Keyboard className="w-5 h-5 text-cyan-400" />
        <div className="absolute -top-8 right-0 text-xs text-cyan-400 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
          Shortcuts (?)
        </div>
      </motion.button>
        ...
        {showHelp && (
          ...
          >
            <motion.div
              ...
              className="bg-black/90 border border-emerald-500/30 rounded-2xl p-6 max-w-md w-full mx-4 shadow-2xl"
            >
              <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={() => setShowHelp(false)} className="p-1 hover:bg-white/10 rounded-lg transition-colors">
              ...
              <div className="mt-6 pt-4 border-t border-white/10 text-center">
                <p className="text-white/40 text-xs">
                  Press <kbd className="px-1.5 py-0.5 text-xs font-mono bg-black/50 border border-white/20 rounded text-emerald-400">?</kbd> anytime to toggle this help
                </p>
              </div>
            </motion.div>
          </motion.div>
        ...
        <AnimatePresence>
          {toasts.map((toast, index) => (
            <motion.div
              ...
              className="flex items-center gap-2 px-4 py-2 bg-black/90 border border-emerald-500/30 rounded-lg shadow-lg backdrop-blur-sm"
